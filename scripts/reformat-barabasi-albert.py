...
# load parameters
with open(str(stamp)+'-k.json', 'r') as j:
	klist = json.load(j)
with open(str(stamp)+'-n0.json', 'r') as j:
	n0list = json.load(j)
											...
											with open(str(stamp)+'-table-comsize-'+str(n)+'.dat', 'w') as cs:
												for prek in klist:
													...
													for pren0 in n0list:
														n0 = pren0*k
														if (i < len(edges)):
															e.write(str(k) + '\t' + str(n0) + '\t' + str(edges[i]) + '\n')
															c.write(str(k) + '\t' + str(n0) + '\t' + str(cost[i]) + '\n')
															p.write(str(k) + '\t' + str(n0) + '\t' + str(powerLaw[i]) + '\n')
															cc.write(str(k) + '\t' + str(n0) + '\t' + str(clusterCoeff[i]) + '\n')
															a.write(str(k) + '\t' + str(n0) + '\t' + str(degreeAss[i]) + '\n')
															exp.write(str(k) + '\t' + str(n0) + '\t' + str(powerLawExponents[i]) + '\n')
															diam.write(str(k) + '\t' + str(n0) + '\t' + str(diameters[i]) + '\n')
															degen.write(str(k) + '\t' + str(n0) + '\t' + str(degeneracy[i]) + '\n')
															nc.write(str(k) + '\t' + str(n0) + '\t' + str(ncoms[i]) + '\n')
															mo.write(str(k) + '\t' + str(n0) + '\t' + str(mods[i]) + '\n')
															cs.write(str(k) + '\t' + str(n0) + '\t' + str(int(n/ncoms[i])) + '\n')
															i = i+1
														else:
															break
													else:
