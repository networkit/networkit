...
if os_soabi is None:
    os_soabi = sysconfig.get_config_var("EXT_SUFFIX").split(".")[
        1
    ]  # get_config_var('SOABI') is None on win32-systems
...
def buildNetworKit(
    ...
    )  # provide python.h files
    comp_cmd.append(
        "-DNETWORKIT_PYTHON_EXECUTABLE=" + sys.executable
    )  # provide cmake with Python interpreter
    comp_cmd.append(
        "-DNETWORKIT_PYTHON_SOABI=" + os_soabi
    )  # provide lib env specification
    ...
    if sys.platform == "win32":
        ...
        )  # Windows only supports static core builds
    ...
    # Run cmake
    print(
        "initializing NetworKit compilation with: '{0}'".format(" ".join(comp_cmd)),
        flush=True,
    )
...
class build_ext(Command):
    ...
    user_options = [
        ...
        ("networkit-external-core", None, "use external NetworKit core library"),
        ("external-tlx=", None, "absolute path to external tlx library"),
    ...
    def finalize_options(self):
        self.set_undefined_options(
            "build", ("build_lib", "build_lib"), ("build_temp", "build_temp")
        )
...
try:
    ...
    print(
        "WARNING: _tkinter is necessary for networkit.profiling module if not used via jupyter.\nInstall tkinter if needed"
    )
