...
class TestGraph(unittest.TestCase):
    ...
    def testAddEdgesCooArrayWithMatrix(self):
        ...
        S = sc.sparse.coo_matrix(
            [[0, 1, 0], [1, 0, 0], [0.0, 0.0, 1.0]], dtype=np.double
        )
    ...
    def testGraphFromGraphEventStream(self):
        ...
        upE1 = nk.dynamics.GraphEvent(
            nk.dynamics.GraphEventType.EDGE_ADDITION, 1, 2, 2.0
        )
        upE2 = nk.dynamics.GraphEvent(
            nk.dynamics.GraphEventType.EDGE_ADDITION, 2, 3, 3.0
        )
        upE3 = nk.dynamics.GraphEvent(
    ...
    def testGraphDifference(self):
        ...
        update1 = nk.dynamics.GraphEvent(
            nk.dynamics.GraphEventType.EDGE_ADDITION, 0, 3, 2.0
        )
        update2 = nk.dynamics.GraphEvent(
    ...
    def testPrimMinimumSpanningForest(self):
        G = self.getSmallGraph()
        pmf = nk.graph.PrimMSF(G)
        pmf.run()
        self.assertEqual(G.numberOfNodes(), pmf.getForest().numberOfNodes())

    def testSpanningForest(self):
        G = self.getSmallGraph()
        sf = nk.graph.SpanningForest(G)
        sf.run()
        self.assertEqual(G.numberOfNodes(), sf.getForest().numberOfNodes())
