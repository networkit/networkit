...
def autoReadGraph(path, **kwargs):
	...
	readerCandidates = {f: r for f, r in readers.items() if r.accepts(headerLines)}
	...
	reader = getReader(readerCandidates[0][1],**kwargs)
...
def extractHeaderLines(path, n):
	...
	if not os.path.isfile(path):
		...
		with open(path, "r") as file:    # catch a wrong path before it crashes the interpreter
			try:
				headerLines = [l.strip() for l in file.readlines(n)]
			except Exception as e:
