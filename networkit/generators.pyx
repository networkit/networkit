...
cdef class BarabasiAlbertGenerator(StaticGraphGenerator):
    ...
    Albert[1], implemented in the much faster method from Batagelj and Brandes[2] per default where the running time is O(n+m). 
    Furthermore there is a parallel version from Sanders and Schulz[3] implemented which can be selected by setting sequential=False which empirically showed better runtimes (when executed multi-threaded).
