...
std::vector<count> GraphletsCounterImpl<GraphletsSize::THREE>::getCounts() const {
    count nb_triangle{0}, nb_2_star{0}, nb_single_edge{0}, nb_empty{0};
    auto V{G->numberOfNodes()};
    G->parallelForEdges( // l. 3
        [this, V, &nb_triangle, &nb_2_star, &nb_single_edge](node u, node v) {
            ...
            G->forNeighborsOf(      // l. 5
                u, [v, &X, &star_u, &nb_neighbours](node w) {
                    if (v == w) // l. 6
                        return;
                    // l. 7
                    star_u.insert(w);
            ...
            G->forNeighborsOf( // l. 8
                v, [u, &X, &triangles, &star_u, &star_v, &nb_neighbours](node w) {
                    if (u == w) // l. 9
                        return;
                    if (X[w] == 1) { // l. 10
                        triangles.insert(w);
...
#pragma omp atomic update
    ...
    return {nb_triangle, nb_2_star, nb_single_edge, nb_empty};
}
...
private:
    ...
    // l. 39-45
    inline count cliqueCount(std::vector<short> &X, const std::set<node> &triangles) const;
    // l. 46-52
    inline count cycleCount(std::vector<short> &X, const std::set<node> &stars) const;
};
...
std::vector<count> GraphletsCounterImpl<GraphletsSize::FOUR>::getCounts() const {
    ...
    count nb_triangle{0},                  // connected 3-graphlets
        nb_2_star{0}, nb_3_single_edge{0}, // disconnected 3-graphlets
        nb_3_empty{0}, nb_4_clique{0},     // connected 4-graphlets
        nb_4_chordalcycle{0}, nb_4_tailedtriangle{0}, nb_4_cycle{0}, nb_3_star{0}, nb_4_path{0},
        nb_4_single_triangle{0}, // disconnected 4-graphlets
        nb_4_single_star{0}, nb_4_two_disconnected_edges{0}, nb_4_single_edge{0}, nb_4_empty{0};
...
#pragma omp atomic update
    ...
    // l. 37
    nb_4_two_disconnected_edges = N_II1 / 2 - 3 * nb_4_clique - 2 * nb_4_chordalcycle
                                  - nb_4_tailedtriangle - 2 * nb_4_cycle - nb_4_path; // Eq. 13
    nb_4_single_edge = N_II - 2 * nb_4_two_disconnected_edges;                        // Lemma 7
    nb_4_empty = binom4(V) - nb_4_clique - nb_4_chordalcycle - nb_4_tailedtriangle - nb_4_cycle
                 - nb_3_star - nb_4_path - nb_4_single_triangle - nb_4_single_star
                 - nb_4_two_disconnected_edges - nb_4_single_edge; // Eq. 14
    return {nb_triangle,
            nb_2_star,
            nb_3_single_edge,
            nb_3_empty,
...
count GraphletsCounterImpl<GraphletsSize::FOUR>::cliqueCount(
    std::vector<short> &X, const std::set<node> &triangles) const {
    count ret{0};
    for (node w : triangles) {
        G->forNeighborsOf(w, [&ret, &X](node r) {
            if (X[r] == 2)
                ++ret;
        });
        X[w] = 0;
