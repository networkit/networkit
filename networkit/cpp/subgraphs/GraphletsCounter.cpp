...
GraphletsCounter::GraphletsCounter(const Graph &G, unsigned K) noexcept : G{&G}, k{K} {}

void GraphletsCounter::run() {
    if (G->isDirected())
        throw std::runtime_error("Graphlets counting is only implemented for undirected graphs");
    ...
    case 4u:
        ...
        break;
    default:
        throw std::runtime_error("No algorithm is provided for " + std::to_string(k)
                                 + "-graphlets counting");
    }
}
} // namespace NetworKit
