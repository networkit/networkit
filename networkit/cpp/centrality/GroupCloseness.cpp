...
edgeweight GroupCloseness::computeImprovement(node u, count n, Graph& G, count h) {
...
std::vector<count> GroupCloseness::newDistances(node u, count n, Graph& G, count h) {
  std::vector<count> d1(n);
	G.forNodes([&](node v){
		d1[v] = d[v];
	});
...
bool pairCompare(const std::pair<node, count>& firstElem, const std::pair<node, count>& secondElem) {
...
void GroupCloseness::run() {
  ...
  omp_init_lock(&lock);
	...
	maxD = 0;
  count sumD = 0;
	// TODO: actually, we could have more generic parallel reduction iterators in the Graph class
	G.forNodes([&](node v) {
		if (d[v] > maxD) {
			maxD = d[v];
		}
        ...
        // Q: fully synchronized;
        {
              while(!Q.empty()) {
                ...
                if (i > 1 && prevBound[v] <= currentImpr) {
                  INFO("Interrupting! currentImpr = ", currentImpr, ", previous bound = ", prevBound[v]);
                  toInterrupt = true;
                  break;
                }
                if (D[v]>1 && !(d[v] == 1 && D[v] == 2) && d[v] > 0 && (i == 1 || prevBound[v] > currentImpr)) {
