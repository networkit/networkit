...
TEST_F(DynTopClosenessGTest, testDynTopHarmonicCloseness) {
  ...
  TopHarmonicCloseness refCentrality(G, k, true);
  refCentrality.run();

  auto scores = centrality.ranking();
  auto refScores = refCentrality.topkNodesList();
  for (count j = 0; j < k; ++j) {
    INFO(scores[j].first, ", ", refScores[j]);
    EXPECT_TRUE(scores[j].first == refScores[j]);
  }
}
...
//      if (!scoreMatch) {
//        INFO(referenceScores[j].first, " - ", scores[j].first);
//      }
