...
TEST_F(CentralityGTest, testKatzCentralityDirected) {
  ...
  Graph G =
      reader.read("input/wiki-Vote.txt"); // TODO: replace by smaller graph
  KatzCentrality kc(G);
...
TEST_F(CentralityGTest, benchCoreDecompositionSnapGraphs) {
  ...
  std::vector<std::string> filenames = {
      "soc-LiveJournal1.edgelist-t0.graph", "cit-Patents.txt",
      "com-orkut.ungraph.txt", "web-BerkStan.edgelist-t0.graph"};
...
TEST_F(CentralityGTest, benchCoreDecompositionDimacsGraphs) {
  METISGraphReader reader;
  std::vector<std::string> filenames = {"eu-2005", "coPapersDBLP", "uk-2002",
                                        "europe-osm", "cage15"};

  for (auto f : filenames) {
    std::string filename("/home/i11/staudt/Graphs/Static/DIMACS/Large/" + f +
                         ".metis.graph");
    DEBUG("about to read file ", filename);
    Graph G = reader.read(filename);
    G.removeSelfLoops();
...
TEST_F(CentralityGTest, benchCoreDecompositionLocal) {
  METISGraphReader reader;
  std::vector<std::string> filenames = {"coPapersCiteseer", "in-2004",
                                        "coAuthorsDBLP", "audikw1"};
