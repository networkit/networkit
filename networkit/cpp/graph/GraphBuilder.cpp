...
GraphBuilder::GraphBuilder(count n, bool weighted, bool directed, bool useWholeEdges)
    : n(n), selfloops(0), weighted(weighted), directed(directed), useWholeEdges(useWholeEdges) {
...
void GraphBuilder::addHalfOutEdge(node u, node v, edgeweight ew) {
    outEdgesPerThread[omp_get_thread_num()][u % omp_get_max_threads()].emplace_back(HalfEdge{u, v});
    if (weighted) {
...
void GraphBuilder::addHalfInEdge(node u, node v, edgeweight ew) {
    inEdgesPerThread[omp_get_thread_num()][u % omp_get_max_threads()].emplace_back(HalfEdge{u, v});
    if (weighted) {
...
void GraphBuilder::swapNeighborhood(node u, std::vector<node> &neighbours,
    ...
    for (node v : neighbours) {
        new_edges.push_back(HalfEdge{u, v});
    }
...
void GraphBuilder::setOutWeight(node u, node v, edgeweight ew) {
    ...
    if (vi != none) {
        ...
        if (!directed && useWholeEdges) { // need to adjust both half edges
...
void GraphBuilder::increaseOutWeight(node u, node v, edgeweight ew) {
    ...
    if (vi != none) {
        outEdgeWeightsPerThread[omp_get_thread_num()][u % omp_get_max_threads()][vi] += ew;
        if (!directed && useWholeEdges && u != v) { // need to adjust both half edges
            index ui = indexInOutEdgeArrayPerThread(v, u);
            outEdgeWeightsPerThread[omp_get_thread_num()][v % omp_get_max_threads()][ui] += ew;
        }
    } else {
            ...
            if (weighted) {
                ...
                for (index j = 0; j < edges.size(); ++j) {
                    G.addPartialOutEdge(Unsafe{}, edges[j].source, edges[j].destination,
                                        weights[j]);
                    if (!useWholeEdges) {
                        inEdgesPerThread[edges[j].destination%max_threads][thread_num].emplace_back(
                            HalfEdge{edges[j].destination, edges[j].source});
                        inEdgeWeightsPerThread[edges[j].destination%max_threads][thread_num].emplace_back(weights[j]);
                    }
                }
            } else {
                for (HalfEdge edge : edges) {
                    ...
                    if (!useWholeEdges) {
                        inEdgesPerThread[edge.destination%max_threads][thread_num].emplace_back(
                            HalfEdge{edge.destination, edge.source});
                    }
                }
            }
        }
