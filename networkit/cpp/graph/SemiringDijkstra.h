...
#ifndef SEMIRINGDIJKSTRA_H_
#define SEMIRINGDIJKSTRA_H_
 ...
 */
template <class T>
class SemiringDijkstra : public APP<T> {
...
public:
	 ...
	 */
    SemiringDijkstra(const Graph& G, node source, std::vector<T> edgeWeights, bool storePaths=true, bool storeStack=false, node target = none) : APP<T>(G, source, storePaths, storeStack, target, edgeWeights) {
