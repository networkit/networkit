...
#pragma omp parallel for
	for (omp_index i = 0; i < cIndexFine.size(); ++i) {
		cIndexFine[i] = i;
	}
...
#pragma omp parallel for
	for (omp_index i = 0; i < xc.getDimension(); ++i) {
...
#pragma omp parallel for
		for (omp_index i = 0; i < bq.getDimension(); ++i) { // bq = s.q .* b.f
			bq[i] = q[i] * bFSet[i];
		}
...
#pragma omp parallel for
		for (omp_index i = 0; i < xFSet.getDimension(); ++i) {
			xf[fSet[i]] = xFSet[i];
		}

		const std::vector<index> &cSet = s.getCSet();
#pragma omp parallel for
		for (omp_index i = 0; i < currX.getDimension(); ++i) {
			xf[cSet[i]] = currX[i];
		}
...
#pragma omp parallel for
	for (omp_index i = 0; i < elements.size(); ++i) {
