...
public:
   ...
   * their distance to the edge modification and, in the case of an edge
   * insertion, an upper bound for the improvement of the harmonic closeness
   * centrality of each affected node.
   *
   * @param G The graph.
   ...
   *
   * @return The improvement upper bound for each affected node, indexed by node
   * ID
   */
...
private:
   ...
   */
  std::vector<edgeweight> bfsWithoutStartNeighbor(node source,
                                                  node startNeighbor);
   ...
   * @return The distances of each node to @a source.
   */
  std::vector<edgeweight> reverseBfsWithoutStartNeighbor(node source,
                                                         node startNeighbor);
  ...
  /**
   * Runs a reverse BFS from @a source while optionally including an edge
   * between
