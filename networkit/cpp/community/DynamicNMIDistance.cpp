...
//	count n = newGraph.numberOfNodes();
	...
	for (omp_index C = oldClustering.lowerBound(); C < oldClustering.upperBound(); ++C) {
		P_old[C] = ((double) size_old[C]) / numDouble;
	}
	#pragma omp parallel for
	for (omp_index C = newClustering.lowerBound(); C < newClustering.upperBound(); ++C) {
...
double DynamicNMIDistance::entropy(const Partition& clustering, count n, std::vector<double> probs) {
	...
	for (omp_index C = clustering.lowerBound(); C < clustering.upperBound(); ++C) {
		if (probs[C] != 0) {
