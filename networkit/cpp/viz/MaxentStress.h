...
public:
	...
	MaxentStress(const Graph& G, const count dim, const count k, LinearSolver<CSRMatrix> &solver, bool fastComputation=false, GraphDistance graphDistance = EDGE_WEIGHT);

	MaxentStress(const Graph& G, const count dim, const std::vector<Point<double>>& coordinates, const count k, LinearSolver<CSRMatrix> &solver, bool fastComputation=false, GraphDistance graphDistance = EDGE_WEIGHT);
